...
module.exports = class SensorDevice extends Homey.Device {
  ...
  async onInit(ignoreWatch) {
    ...
    if (ignoreWatch !== true) {
      ...
      for (let w in this.watches) {
        ...
        let lfunc = val => this.setCapabilityValue(this.watches[w].event, val)
  ...
  setValues() {
    for (let w in this.watches) {
      ...
      this.setCapabilityValue(this.watches[w].event, value)
        .then(() => this.log('Capability', this.watches[w].event, 'has been set to', value))
        .catch((e) => this.log('Error', e, 'setting value', value, 'for', this.watches[w].event))
    }
  }
