...
var eventGetMap = {
	'measure_pressure': (x) => x * 1000,
	'target_temperature': (x) => { 
		let r = values.RemoteOverrideRoomSetpoint;
		return (r != null && r > 0 ? r : values.CurrentSetpoint);
	},
	'thermostat_mode': (x) => (x >= values.CurrentSetpoint ? 'heat' : (x > 0 && x < values.CurrentSetpoint ? 'cool' : 'auto')),
...
var self = module.exports = {
	...
	openComm: function(ip, port) {
		...
		if (ok) {
			...
			client.setTimeout(60000); // there should be communication at least once a minute
		...
		var devices = [{ 
				...
				data: { 
					...
					watch: [
						...
						{ variable: 'RemoteOverrideRoomSetpoint', event: 'thermostat_mode' },
	...
	getThermostatMode: function(device) {
		var state = eventGetMap.thermostat_mode(values.RemoteOverrideRoomSetpoint);
		return state;
	},
