...
var flameText = { 
	'0': { en: "Flame off", nl: "Vlam uit" },
	'1': { en: "Flame on", nl: "Vlam aan" }
}

function otgFlame2text(value) {
	return flameText[value][locale] || flameText[value].en;
}
...
var self = module.exports = {
	...
	openComm: function(ip, port) {
		if (found) {
			// We already found an OTG on this IP; re-connect to this every 3 seconds
	...
	getLoggableVars: function() {
		var list = [];
		// TODO: look at values (now missing override values)
		for (var item in msg_supported) {
			if (ot.Messages[item].flags != null) {
				var src = ot[ot.Messages[item].flags];
				for (var i in src) {
					if (src[i].var != null) {
						var text = src[i].locale || src[i].en;
						list.push({ id: item + '#' + i, var: src[i].var, txt: text });
					}
				}
			} else if (ot.Messages[item].var != null) {
				var text = ot.Messages[item].locale || ot.Messages[item].en;
				list.push({ id: item, var: ot.Messages[item].var, txt: text });
			}
		}
		return list;
	},
	...
	getThermostatState: function(device) {
		var state = self.getValue('thermostat_state', 'StatusFlame');
		return state;
	},
